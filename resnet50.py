...
def identity_block2d(input_tensor, kernel_size, filters, stage, block, is_training, reuse):  #x1, 3, [64, 64, 256], stage=2, block='1b', is_training=is_training, reuse=reuse
    filters1, filters2, filters3 = filters

    conv_name_1 = 'conv' + str(stage) + '_' + str(block) + '_1x1_reduce'
    bn_name_1 = 'bn' + str(stage) + '_' + str(block) + '_1x1_reduce'

    x = tf.layers.conv2d(input_tensor, filters1, (1, 1), use_bias=False, name=conv_name_1, reuse=reuse)  # 1b(?, 56, 56, 64) 2a(?,28,28,128)
    ...
    x = tf.layers.conv2d(x, filters2, kernel_size, padding='SAME', use_bias=False, name=conv_name_2, reuse=reuse)  # 1b(?, 56, 56, 64) 2a(?,28,28,128)
    ...
    x = tf.layers.conv2d(x, filters3, (1, 1), name=conv_name_3, use_bias=False, reuse=reuse)  # 1b(?, 56, 56, 256) 2a(?,28,28,512)
...
def conv_block_2d(input_tensor, kernel_size, filters, stage, block, is_training, reuse, strides=(2, 2)):  #  x, 3, [64, 64, 256], stage=2, block='1a', strides=(2, 2), is_training=is_training, reuse=reuse
    filters1, filters2, filters3 = filters  # ResNet利用了1×1卷积，并且是在3×3卷积层的前后都使用了，不仅进行了降维，还进行了升维，使得卷积层的输入和输出的通道数都减小，参数数量进一步减少

    conv_name_1 = 'conv' + str(stage) + '_' + str(block) + '_1x1_reduce'
    bn_name_1 = 'bn' + str(stage) + '_' + str(block) + '_1x1_reduce'
    x = tf.layers.conv2d(input_tensor, filters1, (1, 1), use_bias=False, strides=strides, name=conv_name_1, reuse=reuse)  # 1a(?,56,56,64) 2a(?,28,28,128)
    x = tf.layers.batch_normalization(x, training=is_training, name=bn_name_1, reuse=reuse)  # 1a(?,56,56,64)
    x = tf.nn.relu(x)

    conv_name_2 = 'conv' + str(stage) + '_' + str(block) + '_3x3'
    bn_name_2 = 'bn' + str(stage) + '_' + str(block) + '_3x3'
    x = tf.layers.conv2d(x, filters2, kernel_size, padding='SAME', use_bias=False, name=conv_name_2, reuse=reuse)  # 1a(?,56,56,64) 2a(?,28,28,128)
    x = tf.layers.batch_normalization(x, training=is_training, name=bn_name_2, reuse=reuse)  # 1a(?,56,56,64)
    x = tf.nn.relu(x)

    conv_name_3 = 'conv' + str(stage) + '_' + str(block) + '_1x1_increase'
    bn_name_3 = 'bn' + str(stage) + '_' + str(block) + '_1x1_increase'
    x = tf.layers.conv2d(x, filters3, (1, 1), name=conv_name_3, use_bias=False, reuse=reuse)  # 1a(?,56,56,256) 2a(?,28,28,512)
    x = tf.layers.batch_normalization(x, training=is_training, name=bn_name_3, reuse=reuse)  # 1a(?,56,56,256) 2a(?,28,28,512)
    ...
    shortcut = tf.layers.batch_normalization(shortcut, training=is_training, name=bn_name_4, reuse=reuse)   # 1a(?,56,56,256) 2a(?,28,28,512)
...
def resnet50(input_tensor, is_training=True, pooling_and_fc=True, reuse=False):
    ...
    x = tf.layers.batch_normalization(x, training=is_training, name='bn1_1/3x3_s1', reuse=reuse)  # (?,112,112,64) bn 输入batch标准化
    x = tf.nn.relu(x)  # (?,112,112,64) 激活函数relu
    # x = tf.layers.max_pooling2d(x, (2,2), strides=(2,2), name='mpool1')

    x1 = conv_block_2d(x, 3, [64, 64, 256], stage=2, block='1a', strides=(2, 2), is_training=is_training, reuse=reuse)  # 先stride2一次减小，L:112>>56，D:3>>64升维,k=1；再卷一次L:56>>56，D:64>>64,k=3捕获像素八邻域信息；再卷一次加深L:56>>56，D:64>>256二次升维,k=1；123串行产生A，4input图上sdride2减小为fmap大小，L:112>>56，D:3>>256,k=1产生B；5A+B对应元素相加[升维，拓宽，升维，+x]
    x1 = identity_block2d(x1, 3, [64, 64, 256], stage=2, block='1b', is_training=is_training, reuse=reuse)  #三次卷积stride都=1，k分别为[1,3,1]，D分别为 [64,64,256]，[降维，拓宽，升维]
    x1 = identity_block2d(x1, 3, [64, 64, 256], stage=2, block='1c', is_training=is_training, reuse=reuse)  #三次卷积stride都=1，k分别为[1,3,1]，D分别为 [64,64,256]，[降维，拓宽，升维]
    ...
    x2 = identity_block2d(x2, 3, [128, 128, 512], stage=3, block='2d', is_training=is_training, reuse=reuse)  # (?,28,28,512)
    ...
    x3 = identity_block2d(x3, 3, [256, 256, 1024], stage=4, block='3f', is_training=is_training, reuse=reuse)  # (?,14,14,1024)
    ...
    x4 = identity_block2d(x4, 3, [512, 512, 2048], stage=5, block='4c', is_training=is_training, reuse=reuse)  # (?,7,7,2048)
